...
app.post('/auth/register_old', async (req, res) => {
    try {
 ...
 *
 * La petición (request) llega desde el cliente con los datos: nombre, correo, usuario, edad, y contraseña.
 * El servidor responde (response) al cliente con el código
 *  -   201 si el usuario fue creado correctamente
 *  -   400 si algún dato no se pudo parsear correctamente
 *  -   409 si el correo o nombre de usuario ya había sido registrado anteriormente
...
app.post('/auth/register', async (req, res) => {
    try {
        ...
        // Verificar que el correo y el usuario no esté registrado
        if (await User.findUserOrEmail(normalizedEmail)) {
            return res.status(409).json({message: 'Ese email ya está registrado.'});
        }
        if (await User.findUserOrEmail(finalUsername)) {
    ...
    try {
        ...
        const {rows} = await pool.query(
            `INSERT INTO comment_votes (comment_id, user_id, useful)
             ...
             DO
            UPDATE SET useful = EXCLUDED.useful
                RETURNING *`,
    ...
    try {
        ...
        const {rows} = await pool.query(
            `INSERT INTO ratings (product_slug, user_id, stars)
             ...
             DO
            UPDATE SET stars = EXCLUDED.stars
                RETURNING *`,
